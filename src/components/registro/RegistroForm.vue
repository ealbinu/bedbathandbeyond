...
q-form(@submit.prevent="submit").row.q-pa-xl.q-col-gutter-sm
    ...
    .col-12
        q-input(v-model="user.Email" required autocomplete="username" type="email" label="Correo electrónico *" v-bind="$inputAttr" :rules="[ val => val && val.length > 3 || 'Ingresa una dirección válida.']" ref="Email")
    ...
    template(v-if="zipcodeOps.length>0")
        .col-12
